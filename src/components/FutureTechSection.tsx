  ...
  const technologies = [
    {
      ...
      titleKey: 'future.bateriasLitio',
      descKey: 'future.bateriasDesc'
    },
    {
      icon: <Zap className="h-5 w-5 text-purple-600" />,
      titleKey: 'future.inversoresHibridos',
      descKey: 'future.inversoresDesc'
    },
    {
      icon: <Car className="h-5 w-5 text-purple-600" />,
      titleKey: 'future.carregadoresVeiculares',
      descKey: 'future.carregadoresDesc'
    }
  ];
        ...
        <ScrollAnimation>
          <div className="mb-16">
            <div className="flex items-center mb-2">
              ...
              <span className="text-purple-600 uppercase tracking-wider text-sm font-medium">{t('future.title')}</span>
            </div>
            <h2 className="text-4xl lg:text-6xl font-bold text-gray-900 mb-6 leading-tight">
              {t('future.heading')}
            </h2>
            <p className="text-2xl text-gray-600 max-w-6xl font-medium">
              {t('future.subtitle')} <span className="text-purple-600">{t('future.subtitleBold')}</span> {t('future.subtitleEnd')}
            </p>
          </div>
        </ScrollAnimation>
      ...
      >
              ...
              <div className="bg-black/30 backdrop-blur-sm rounded-lg p-2 md:p-6 max-w-xs w-full md:max-w-md border border-white/20">
                ...
                <div className="grid grid-cols-1 gap-1 md:gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="font-semibold text-white text-xs md:text-xs">{t('future.backupEnergia')}</h4>
                      <p className="text-gray-200 text-[10px] md:text-xs">{t('future.backupDesc')}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Car className="h-3 w-3 md:h-4 md:w-4 text-white" />
                    <div>
                      <h4 className="font-semibold text-white text-xs md:text-xs">{t('future.mobilidadeGarantida')}</h4>
                      <p className="text-gray-200 text-[10px] md:text-xs">{t('future.mobilidadeDesc')}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Plug className="h-3 w-3 md:h-4 md:w-4 text-white" />
                    <div>
                      <h4 className="font-semibold text-white text-xs md:text-xs">{t('future.integracaoInteligente')}</h4>
                      <p className="text-gray-200 text-[10px] md:text-xs">{t('future.integracaoDesc')}</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <ScrollAnimation delay={1200} animationClass="reveal-fade-up">
          <div className="border-l-4 border-purple-600 pl-6 py-4">
            <h3 className="text-lg font-semibold text-gray-900 mb-3">{t('future.revolucaoTitle')}</h3>
            <p className="text-gray-600 mb-4 leading-relaxed">
              {t('future.revolucaoDesc')}
            </p>
            <p className="text-sm text-purple-600 font-medium">{t('future.revolucaoFooter')}</p>
          </div>
        </ScrollAnimation>
      </div>
    </section>
